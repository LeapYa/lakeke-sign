...
say "       2) 把微信钉回已知可用的旧版——本项目 fetch_wechat_deb.sh 按版本下载并校验 sha256："
say "          bash fetch_wechat_deb.sh 4.1.13.23 ./wechat-cdn"
say "          再把该目录挂到静态服务，启动实例时加 -e WECHAT_CDN=https://<你的镜像>/weixin/Universal/Linux"
say "       3) 自己算偏移（本项目自带工具，不用等上游补配置）："
say "          bash auto_offsets.sh <实例容器名>     # 一线命令：抠二进制 -> 算 -> 装进 WMPFDebugger"
say "          详见 offsets/README.md"
