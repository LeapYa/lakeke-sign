...
`reopen_miniapp.py` 自己就能打开：

1. 点侧边栏顶部那组**最后一个**按钮 → 弹出**小程序面板**（独立窗口）
2. 面板右上角放大镜 → 输入 **辣可可现炒黄牛肉** → **按回车** → 面板里开出 `辣可可现炒黄牛肉_搜索`
3. 在结果卡片里点 **辣可可现炒黄牛肉i** → 小程序窗口打开（微信把窗口标题设成小程序名）

> **注意结尾那个 `i`**：搜索会返回两个同名号 —— `辣可可现炒黄牛肉`（扫码点餐，那是**餐饮/商城**号，
> 首页是五常米海报）和 `辣可可现炒黄牛肉i`（「为顾客提供便捷的点餐服务」，**签到要的是这个**，
> 只有它显示 `积分 / NQ=卡号`，卡号应与 `lakeke.env` 的 `LAKEKE_CARDNO` 一致）。
> 脚本按**窗口标题精确等于**判定，开错了会关掉继续试下一张卡片。

这条路走的是**商店搜索**，不依赖账号历史 —— 新号、从没打开过的号一样能搜到，
所以**不需要「首次人工打开一次」**。

`wx.login` 的 jsCode 与 appid 绑定（实测甄选产的 code 配辣可可 mpId 一律 `invalid code`），
...
> 想手动签一次的话，入口在**辣可可甄选**首页：搜 **辣可可甄选** → 打开 → 点首页横幅左下角
> **「点击签到」** → 弹「即将打开 辣可可现炒黄牛肉」→ 允许。自动化不需要走这条路。
...
按 `RESULT=<code>` 判定 → **关掉小程序与面板** → 通知。

**最后那步「关掉」是为了省内存**。微信的小程序运行时是这套方案里最占内存的部分，实测（容器内存）：
...
也就是说「用完即关」把内存峰值压到**每天只出现一次**（签到那 1~2 分钟），其余时间回到低谷。
代价是每次签到要多花约 1 分钟重新打开小程序 —— 如果你更在意这点时间，设 `LAKEKE_KEEP_OPEN=1`
让它保持常开（那内存就一直是高位）。

> 大头省不掉：2.48 GiB 里绝大部分是微信本体与 WMPF 常驻框架，只有关掉整个微信才能释放，
> 而重启微信会让登录态回到「点一下登录」的界面 —— 但**点完必须在手机上确认**才算登录成功
> （Linux 版微信没有 Windows 的「登录免确认」选项），久不确认还会过期、转为只能扫码。
> 无人值守时这个风险不值得冒。
> 所以默认策略是「关小程序、留微信」。
...
| 小程序上下文查不到 | 小程序被关掉了 | `daily.sh` 会自动重开（面板搜索 → 结果卡片）；开错了它会关掉再试下一张 |
