...
# 终端 2：签到（自动开小程序 → 刷 token → 签到）
#         前提：微信**已经登录**。掉登录时点完「登录」还得你在手机上确认（见 5. 会话保活）
cd lakeke-sign
python sign_now.py
```
...
> ⚠️ 微信自身的登录确认躲不掉：**点完「登录」按钮必须在手机上确认**才算登录成功。
> Windows 版微信有「登录免确认」选项可以免掉这一步（不在这个 Linux 方案里）；
> **Linux 版客户端没有这个选项**，而且手机久不确认会提示「登录状态已过期」→ 之后只能扫码。
> 所以「别让微信掉登录」比「掉了再自动救回来」重要得多。
