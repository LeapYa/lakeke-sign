...
辣可可小程序「可可会员签到」的每日自动签到。Windows 本机可跑；无人值守首推 Linux 容器。

## 能做什么

- **自动签到**：在微信里现取 token，再调签到接口，全程不用手工抓包
- **凭证自动续**：token 只有 1~2 小时，脚本在小程序逻辑层调 `wx.login` 换新的
- **无人值守**：`daily.sh` 一条命令完成自检 → 自愈 → 刷新 token → 签到 → 通知
- **失败告警**：支持企业微信机器人 / PushPlus / WXPusher / 钉钉 / 邮件 / 通用 webhook
...
**首推 Linux 容器**：微信 Linux 版不会自动升级，WMPF 版本能钉住；机器常开即可，不用管你的电脑。
...
- token 1~2 小时过期（服务端校验 `exp`），jsCode 只能由微信客户端产生，所以 GitHub Actions
  这类纯云端跑不了，必须有微信环境现取现用。详见 [DEPLOY.md](DEPLOY.md) 第五节。
